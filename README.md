...
- **Dynamic Password Download**: Downloads require a time-based dynamic password (SHA256 hash of UTC hour + app name)
...
let password = calculate_dynamic_password(&hour_seed, "upload-download");
```
...
| `/api/download/{filename}` | GET | Download file (requires `?password=`) |
| `/api/delete/{filename}` | DELETE | Delete file (requires `?password=`) |
| `/api/verify` | POST | Verify dynamic password |
| `/api/lang` | GET | Get translations (use `X-Lang` header) |

## Security

- **Dynamic Password**: Changes every hour, based on UTC time
- **Rate Limiting**: 3 failed attempts per IP within 10 minutes
...
├── examples/
│   └── genpwd.rs        # Password generator example
